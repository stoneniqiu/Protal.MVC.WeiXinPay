...
namespace WxPayAPI
{
    public class WxPayApi
    {
        ...
        */
        ...
        public static WxPayData Sendredpack(WxPayData inputData, int timeOut = 30)
        {
            var url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
            inputData.SetValue("wxappid", WxPayConfig.APPID);//公众账号ID
            inputData.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            inputData.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            inputData.SetValue("sign", inputData.MakeSign());//签名
